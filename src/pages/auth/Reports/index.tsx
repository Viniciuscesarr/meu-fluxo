...
export default function Reports() {
  ...
  const pieData = useMemo(() => {
    ...
    // Agregar fatias pequenas em "Outros" (máx. 5)
    if (arr.length <= 5) return arr;
    const sorted = [...arr].sort((a, b) => b.value - a.value);
    const top = sorted.slice(0, 4);
    const rest = sorted.slice(4);
    const others = { id: -1, value: rest.reduce((s, i) => s + i.value, 0), label: 'Outros' };
    return [...top, others];
  }, [filtered]);
  ...
  const downloadCSV = () => {
    ...
    a.href = url; a.download = `relatorio-${month}-${year}.csv`; a.click();
    URL.revokeObjectURL(url);
  };

  const downloadPDF = async () => {
    // Geração simples via window.print() de uma view mínima
    ...
    w.document.write(`<!doctype html><html><head><title>Relatório ${month}/${year}</title>
      <style>
      ...
      <table>
        <thead><tr><th>Data</th><th>Tipo</th><th>Valor</th><th>Carteira</th><th>Categoria</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
  ...
  const downloadPDFAll = async () => {
    ...
    const totalIncome = allSorted.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0);
    const totalExpense = allSorted.filter(t => t.type === 'expense').reduce((s, t) => s + t.amount, 0);
    w.document.write(`<!doctype html><html><head><title>Relatório Completo</title>
      <style>
        body{font-family:Arial;padding:16px}
        h1{font-size:18px}
        table{width:100%;border-collapse:collapse;margin-top:12px}
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 mt-20">
        ...
        <div className="bg-white p-4 rounded-lg shadow mb-4 flex gap-3 items-center border border-gray-100">
          <input className="border rounded px-3 py-2 w-20" value={month} onChange={(e) => setMonth(e.target.value)} placeholder="MM" />
          <input className="border rounded px-3 py-2 w-28" value={year} onChange={(e) => setYear(e.target.value)} placeholder="AAAA" />
          <div className="flex gap-2 ml-auto">
            <button type="button" className="px-3 py-2 rounded bg-gray-200 hover:bg-gray-300" onClick={downloadCSV}>Exportar CSV</button>
            <button type="button" className="px-3 py-2 rounded bg-gray-200 hover:bg-gray-300" onClick={downloadPDF}>Exportar PDF</button>
            <button type="button" className="px-3 py-2 rounded bg-gray-200 hover:bg-gray-300" onClick={downloadPDFAll}>PDF Completo</button>
          </div>
        </div>
            ...
            {pieData.length > 0 ? (
              <div className="flex flex-col items-center">
                <PieChart
                  ...
                  height={340}
                />
                <div className="mt-2 flex flex-wrap justify-center gap-x-3 gap-y-1 text-xs text-gray-600 max-w-[360px]">
                  {pieData.map((p) => (
